...
public class Model extends Observable {
    ...
    public boolean moveTileUpAsFarAsPossible( int x , int y, Set<Tile> mergedTiles) {
        ...
        while (y < board.size() - 1) {
            ...
            if (nextTile != null) {
                if (nextTile.value() == myValue  && !mergedTiles.contains(nextTile) ) {
                    targetY = row;
                } else {
                    targetY = y;
                }
    ...
    public boolean tiltColumn(int x) {
        ...
        for (int row = board.size() - 1; row >= 0; row--) {
            ...
            int size = board.size();
            if ((tile(x, row) != null) && (row != board.size() - 1)) {
    ...
    /**
     * The helper function of  atLeastOneMoveExists()
     * to check if there are two adjacent tiles with the same value.
     */
    public static boolean TwoAdjacentTileExists(Board b) {
        int size = b.size();
        for (int row = 0; row < size; row++) {
            for (int col = 0; col < size; col++) {
                int currentValue = b.tile(row, col).value();
                // we only need to check one of right/left, top/bottom, because they are same
                // Check right neighbor
                if (col + 1 < size && b.tile(row, col + 1).value() == currentValue) {
                    return true;
                }
                // Check bottom neighbor
                if (row + 1 < size && b.tile(row + 1, col).value() == currentValue) {
                    return true;
                }
            }
        }
    ...
    public static boolean atLeastOneMoveExists(Board b) {
        if (emptySpaceExists(b)){
            return true;
        }else return TwoAdjacentTileExists(b);
        // simply the if-else progress
    }
